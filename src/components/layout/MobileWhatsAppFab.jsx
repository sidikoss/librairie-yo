...
export default function MobileWhatsAppFab() {
  return (
    <div className="fixed bottom-5 right-5 z-50 md:bottom-7 md:right-7">
      {/* Glow ring */}
      <span className="pointer-events-none absolute inset-0 rounded-full bg-[#25D366]/20 blur-md animate-pulse-soft" aria-hidden="true" />
      <a
        href={buildWhatsAppUrl("Bonjour Librairie YO, je veux des informations sur vos livres.")}
        target="_blank"
        rel="noreferrer"
        className="group relative inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white ring-4 ring-white/90 shadow-xl shadow-guinea-400/40 transition-all duration-300 hover:-translate-y-1 hover:scale-110 hover:shadow-2xl hover:shadow-guinea-400/50 focus:outline-none focus-visible:ring-4 focus-visible:ring-brand-300"
        title="Commander via WhatsApp"
        aria-label="Commander via WhatsApp"
      >
        <svg viewBox="0 0 24 24" aria-hidden="true" className="h-7 w-7 fill-current drop-shadow-sm">
